...
DOCKER_SWITCH_BUILD := docker run --rm -e DROPBOX_APP_KEY="$(DROPBOX_APP_KEY)" -e DROPBOX_BRIDGE_BASE="$(DROPBOX_BRIDGE_BASE)" -v $(CURDIR):/work -w /work devkitpro/devkita64 make DROPBOX_APP_KEY="$(DROPBOX_APP_KEY)" DROPBOX_BRIDGE_BASE="$(DROPBOX_BRIDGE_BASE)"
