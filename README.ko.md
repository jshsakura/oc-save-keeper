...
- **속도 제한 및 공격 탐지**: 브리지 남용을 줄이기 위한 백엔드 보호
- **OAuth state 검증**: Dropbox 콜백에 대한 CSRF 완화
- **SSRF 완화 poll URL 처리**: 브리지 polling URL은 설정된 브리지 base로 제한
...
*Dropbox가 OAuth Device Code Grant를 공식 지원하지 않아, Switch에서 폴링 기반 인증을 완료하려면 이 브릿지가 필요합니다.*

> **보안 참고**: 이 브리지는 OAuth `state` 검증, HMAC 보호 poll token, TTL 기반 세션 만료, 1회 consume, 속도 제한 같은 최소한의 보호조치를 포함합니다. 하지만 그렇다고 공용 또는 공유 브리지가 신뢰 가능한 서버가 되는 것은 아닙니다.
>
> 제3자가 운영하는 브리지를 사용하면 OAuth 트래픽과 세션 메타데이터가 내가 통제하지 않는 인프라를 통과합니다. 브리지가 refresh token을 저장하지는 않더라도, 운영자는 접속 메타데이터와 인증 시점 등을 관찰할 수 있습니다. 공용/공유 브리지를 신뢰할지 여부는 사용자 책임이며, 프라이버시가 중요하다면 반드시 자신의 도메인에서 self-host 하고 필요한 경우가 아니면 로깅을 꺼 두는 편이 안전합니다.
