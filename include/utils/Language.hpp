    ...
    void loadBuiltIn(const std::string& langCode) {
        ...
        if (m_currentLang == "ko") {
            assign({
                ...
                {"auth.title", "클라우드 연동설정"},
                {"auth.start_login", "로그인 시작 (QR 생성)"},
                {"auth.status_authenticated", "이미 클라우드에 연결되어 있습니다."},
                {"auth.status_ready", "클라우드 연결을 시작할 준비가 되었습니다."},
                ...
                {"auth.status_success", "클라우드에 성공적으로 연결되었습니다!"},
                {"auth.status_consume_failed", "토큰 교환에 실패했습니다. 다시 시도해 주세요."},
                {"auth.confirm_logout", "정말 클라우드 연결을 해제하시겠습니까?"},
                {"auth.qr_hint", "스캔하여 로그인"},
                {"auth.qr_failed", "QR 생성 실패"},
                {"auth.waiting_bridge", "서버 응답 대기 중..."},
                {"auth.step2_short", "스마트폰을 통해 링크를 열고 Dropbox에 로그인합니다."},
                {"auth.step2_cont", "웹 페이지에서 로그인을 진행하고 권한을 승인합니다."},
                {"auth.step4_short", "승인 및 연동에 성공하면 기기에 로그인 정보가 저장됩니다."},
                {"auth.step4_cont", "(단 최초 로그인시 인증 트래픽이 제3자의 서버를 경유)"},
                {"history.title", "버전 이력 목록"},
                {"history.local", "로컬 백업"},
                {"history.cloud", "클라우드 백업 목록"},
                {"history.synced", "동기화됨"},
                ...
                {"ui.refresh", "새로고침"},
                {"ui.cloud_refresh_hint", "X 버튼: 클라우드 목록 새로고침"},
                {"ui.retry", "재시도"},
                {"ui.revision", "버전"},
                {"ui.device", "기기"},
                {"ui.source", "출처"},
                ...
                {"ui.no", "아니요"},
                {"ui.confirm", "확인"},
                {"ui.auth_failed", "클라우드 인증 교환에 실패했습니다."},
                {"ui.auth_footer_new", "휴대폰으로 QR 코드를 스캔하여 로그인하세요."},
                {"ui.fullscreen_hint", "전체 화면 레이아웃 적용됨"},
                {"ui.action_backup_hint", "로컬 백업을 생성"},
                {"ui.action_upload_hint", "최신 백업 생성 후 클라우드 업로드"},
                {"ui.action_history_hint", "로컬 백업 이력 열기"},
                {"ui.action_cloud_hint", "클라우드 백업 목록 열기"},
        ...
        assign({
            ...
            {"card.state.cloud_newer", "Cloud newer"},
                {"auth.title", "Cloud Setup"},
            {"auth.start_login", "Start Login"},
                {"auth.status_authenticated", "You are already connected to cloud."},
                {"auth.status_ready", "Ready to start cloud connection."},
            ...
            {"auth.status_expired", "Session expired. Please try again."},
                {"auth.status_success", "Successfully connected to cloud!"},
            {"auth.status_consume_failed", "Failed to exchange tokens. Please try again."},
                {"auth.confirm_logout", "Are you sure you want to disconnect from cloud?"},
            {"auth.qr_hint", "Scan to sign in"},
            {"auth.qr_failed", "QR Code Failed"},
            {"auth.waiting_bridge", "Waiting for bridge..."},
            {"auth.step2_short", "Open the link on your phone or PC,"},
            {"auth.step2_cont", "then finish sign-in and approval there."},
            {"auth.step4_short", "Once approved, connection completes"},
            {"auth.step4_cont", "automatically on this device."},
            ...
            {"ui.device_shared_tag", "(Shared)"},
            {"ui.current", "Current"},
            {"ui.selection_footer", "The title list will rescan for the selected save type."},
            {"ui.user_changed", "Selection changed."},
            {"ui.confirm_delete", "Confirm Delete"},
            {"ui.confirm_delete_hint", "Delete immediately after moving to Yes."},
            {"ui.confirm_restore", "Confirm Restore"},
            {"ui.confirm_restore_hint", "Save will be restored immediately. Please be careful."},
            {"ui.no_user", "No user"},
            {"ui.no_save_entries", "No save entries"},
            {"ui.no_revision_entries", "No revision entries"},
            {"ui.refresh", "Refresh"},
            {"ui.cloud_refresh_hint", "X: Refresh cloud list"},
            {"ui.retry", "Retry"},
            ...
            {"ui.size", "Size"},
            {"ui.empty", "Empty"},
            {"ui.no_local_backup_chip", "No Local"},
            ...
            {"ui.no", "No"},
            {"ui.confirm", "Confirm"},
                {"ui.auth_failed", "Cloud authorization exchange failed."},
            ...
            {"ui.action_backup_hint", "Create a local backup"},
                {"ui.action_upload_hint", "Create latest backup, then upload to cloud"},
