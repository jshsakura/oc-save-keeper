    ...
    void loadBuiltIn(const std::string& langCode) {
        ...
        if (m_currentLang == "ko") {
            assign({
                ...
                {"auth.step3", "3. 로그인 링크를 열고 앱 승인을 완료"},
                {"auth.step4", "4. 인증 코드 또는 돌아온 URL을 복사해 아래에 입력:"},
        ...
        assign({
            ...
            {"auth.step2_access", "- Access: App folder"},
            {"auth.step2_name", "- Name: OCSaveKeeper-Backup"},
            {"auth.step3", "3. Open the login link and approve the app"},
            {"auth.step4", "4. Copy the code or redirected URL and paste it below:"},
