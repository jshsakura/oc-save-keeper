...
기기에서 Dropbox 로그인 후 콜백 URL/코드를 수동 복사하지 않고, 브리지 서버가 콜백을 받아 앱이 폴링으로 완료할 수 있게 하는 구조를 정의합니다.
...
## OAuth 처리 흐름 (완전 자동 - 수동 코드 입력 없음)

1. 기기 -> `POST /v1/sessions/start`
2. 브리지 -> PKCE `code_verifier` + `code_challenge` 생성 후 `session_id`, `poll_token`, `authorize_url` 반환
3. 사용자 -> 휴대폰/PC 브라우저에서 `authorize_url` 승인
4. Dropbox -> `GET /oauth/dropbox/callback?code=...&state=...` 호출
5. 기기 -> `POST /v1/sessions/{id}/status` 자동 폴링
6. 승인 상태면 -> `POST /v1/sessions/{id}/consume`
7. 브리지 -> `authorization_code` + `code_verifier` 1회 반환
8. **기기가 Dropbox 토큰 엔드포인트로 직접 교환** → 리프래시 토큰이 기기에서 생성 및 저장

**핵심 포인트:**
- 수동 코드 복사/붙여넣기 불필요 - 자동 폴링으로 완료
- PKCE 키 교환이 브리지에서 자동으로 처리됨
- **리프래시 토큰은 기기에서만 생성 및 저장됨**
- **브리지는 access/refresh 토큰을 절대 저장하지 않음** - 일시적 세션 데이터만 보관
...
- Dropbox API 직접 호출은 콜백 단계에서 없음(토큰 교환을 기기가 수행)
